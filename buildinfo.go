...
func buildInfoHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		...
		fmt.Fprintf(w, "Go toolchain version: %s<br>", bi.GoVersion)
		fmt.Fprintf(w, "Main package path: %s<br>", bi.Path)
		var e [][]string
...
func biTable(w http.ResponseWriter, h2, t1, t2 string, l [][]string) {
	...
	for _, s := range l {
		fmt.Fprintf(w, "<tr><td>%s</td><td style=\"text-align: end\">%s</td></tr>", s[0], s[1])
	}
